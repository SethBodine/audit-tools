...
RUN SODIUM_INSTALL=system pip install --no-cache-dir pynacl; sudo pip install --no-cache-dir azure-cli aws-list-all
...
RUN virtualenv -p python3 venv && venv/bin/pip install --no-cache-dir --upgrade pip && venv/bin/pip install --no-cache-dir prowler-cloud 
# Drop scripts
COPY ./prowler3.sh .
...
# Build AWS List All
RUN pip install --no-cache-dir aws-list-all
